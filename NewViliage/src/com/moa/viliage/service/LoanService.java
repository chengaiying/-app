...
public class LoanService {
	...
	public static ResultInfo<List<FiProd>> prodList(Controller controller){
		...
		List<Record> fiProdRecords = Db.find("select s.* from (SELECT F.* FROM BANK B INNER JOIN fi_prod F ON B.id=F.bank_id AND F.STATUS=0) s \n"
				+ "where ((province IS NULL || LENGTH(trim(province))<1) and (city IS NULL || LENGTH(trim(city))<1) and \n"
				+ "(district IS NULL || LENGTH(trim(district))<1 ))or ( province=? and (city IS NULL || LENGTH(trim(city))<1) and (district IS NULL || LENGTH(trim(district))<1 ) \n"
				+ ")or ( province=? and city=? and  (district IS NULL || LENGTH(trim(district))<1 ))or (province=? and city=? and \n"
...
/*		String startDate = controller.getPara(PARAM_START_DATE);
	...
	public static ResultInfo<FiProd> prodDetail(Controller controller) {
		...
		//xf updated by 20170503
		FiProdLoader.getInstance().loadFiProds();
		FiProd prod = FiProdLoader.getInstance().findProd(prodId);
		//对于新增加的数据，缓存里没有，直接从数据库查询，
		if (prod == null) {
			Record record=Db.findFirst("select * from fi_prod where id=?", prodId);
			if(null!=record){
				prod=FiProd.createProd(record);
			}
			else{
